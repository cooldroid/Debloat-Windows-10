...
Function RemMostApps {
    If ($allusers) {  
        ...
        Get-AppxPackage -AllUsers | where-object {$_.name -notlike "*Store*" -and $_.name -notlike "*Calculator*" -and $_.name -notlike "*sticky*" -and $_.name -notlike "*Windows.Photos*" -and $_.name -notlike "*SoundRecorder*" -and $_.name -notlike "*MSPaint*"} | Remove-AppxPackage -erroraction silentlycontinue
        Get-AppxPackage -AllUsers | where-object {$_.name -notlike "*Store*" -and $_.name -notlike "*Calculator*" -and $_.name -notlike "*sticky*" -and $_.name -notlike "*Windows.Photos*" -and $_.name -notlike "*SoundRecorder*" -and $_.name -notlike "*MSPaint*"} | Remove-AppxPackage -erroraction silentlycontinue
    ...
    Else {
        ...
        Get-AppxPackage | where-object {$_.name -notlike "*Store*" -and $_.name -notlike "*Calculator*" -and $_.name -notlike "*sticky*" -and $_.name -notlike "*Windows.Photos*" -and $_.name -notlike "*SoundRecorder*" -and $_.name -notlike "*MSPaint*"} | Remove-AppxPackage -erroraction silentlycontinue
        Get-AppxPackage | where-object {$_.name -notlike "*Store*" -and $_.name -notlike "*Calculator*" -and $_.name -notlike "*sticky*" -and $_.name -notlike "*Windows.Photos*" -and $_.name -notlike "*SoundRecorder*" -and $_.name -notlike "*MSPaint*"} | Remove-AppxPackage -erroraction silentlycontinue
    }        
}
...
Function RemTasks {
    ...
    Else {
        ...
        Get-Scheduledtask "SmartScreenSpecific", "Microsoft Compatibility Appraiser", "MNO Metadata Parser", "Consolidator", "KernelCeipTask", "UsbCeip", "Microsoft-Windows-DiskDiagnosticDataCollector", "GatherNetworkInfo", "QueueReporting" -erroraction silentlycontinue | Disable-scheduledtask
    }
}
...
Function DisService {
    ...
    Else {
        ...
        #Get-Service diagnosticshub.standardcollector.service, Diagtrack, OneSyncSvc, lfsvc, MapsBroker, NetTcpPortSharing, RemoteAccess, RemoteRegistry, SharedAccess, TrkWks, WbioSrvc, XblAuthManager, XblGameSave, XboxNetApiSvc, WMPNetworkSvc -erroraction silentlycontinue | stop-service -passthru | set-service -startuptype disabled
